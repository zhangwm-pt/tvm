...
from . import conv2d_1x1, conv2d_common
...
def _get_default_config(
    ...
    if is_depthwise:
        ...
        if is_kernel_1x1:
            conv2d_1x1._fallback_schedule(cfg, wkl)
        else:
            conv2d_common._fallback_schedule(cfg, wkl)


@conv2d_infer_layout.register("riscv")
...
@autotvm.register_topi_compute("conv2d_NCHWc.riscv")
...
@autotvm.register_topi_schedule("conv2d_NCHWc.riscv")
def schedule_conv2d_NCHWc(cfg, outs):
    ...
    def _callback(op):
        if "conv2d_NCHWc" in op.tag:
            ...
            if kh == 1 and kw == 1:
                conv2d_1x1._schedule_conv_NCHWc(*args)
            else:
                conv2d_common._schedule_conv_NCHWc(*args)
