...
"""Conv2D alter op and legalize functions for riscv"""
...
@conv2d_alter_layout.register("riscv")
def _alter_conv2d_layout(attrs, inputs, tinfos, out_type):
    ...
    if topi_tmpl == "conv2d_NCHWc.riscv":
        # we only convert conv2d_NCHW to conv2d_NCHWc for riscv
    ...
    if topi_tmpl == "conv2d_NCHWc_int8.riscv":
    ...
    if topi_tmpl == "depthwise_conv2d_NCHWc.riscv":
        if data_layout == "NCHW" and kernel_layout == "OIHW":
            if cfg.is_fallback:
                _get_default_config(
                    cfg,
...
@conv2d_legalize.register("riscv")
