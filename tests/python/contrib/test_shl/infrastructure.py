...
class Device:
    """
    Configuration for SHL tests.

    Check tests/python/contrib/shl/ for the presence of an test_config.json file.
    ...
    def load(cls, file_name):
        ...
        to the test_shl directory.
        """
        location = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
        config_file = os.path.join(location, file_name)
        if not os.path.exists(config_file):
            warnings.warn("Config file doesn't exist, resuming SHL tests with default config.")
            return
...
def skip_runtime_test():
    ...
    # SHL codegen not present.
    if not tvm.get_global_func("relay.ext.shl", True):
        print("Skip because SHL codegen is not available.")
        return True

    # Remote device is in use or SHL runtime not present
    # Note: Ensure that the device config has been loaded before this check
    if not Device.connection_type != "local" and not shl.is_shl_runtime_enabled():
        print("Skip because runtime isn't present or a remote device isn't being used.")
        return True


def skip_codegen_test():
    """Skip test if it requires the SHL codegen and it's not present."""
    if not tvm.get_global_func("relay.ext.shl", True):
        print("Skip because SHL codegen is not available.")
        return True


def build_module(mod, target, params=None, enable_shl=True, tvm_ops=0, shl_partitions=1):
    """Build module with option to build for SHL."""
    ...
    with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
        if enable_shl:
            ...
            for global_var in mod.get_global_vars():
                if "shl" in global_var.name_hint:
                    partition_count += 1

            assert shl_partitions == partition_count, "Got {} SHL partitions, expected {}".format(
                partition_count, shl_partitions
            )
...
def build_and_run(
    ...
    try:
        lib = build_module(mod, device.target, params, enable_shl, tvm_ops, shl_partitions)
...
def extract_shl_modules(module):
    """Get the SHL module(s) from llvm module."""
    return list(filter(lambda mod: mod.type_key == "shl", module.get_lib().imported_modules))
...
def verify_codegen(
    ...
    """Check shl codegen against a known good output."""
    module = build_module(module, target, tvm_ops=tvm_ops, shl_partitions=num_shl_modules)
    shl_modules = extract_shl_modules(module)

    assert len(shl_modules) == num_shl_modules, (
        f"The number of SHL modules produced ({len(shl_modules)}) does not "
        f"match the expected value ({num_shl_modules})."
    )

    for mod in shl_modules:
