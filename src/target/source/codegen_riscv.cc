...
void CodeGenRiscv::Init(bool output_ssa) { print_ssa_form_ = output_ssa; }

void CodeGenRiscv::InitFuncState(const PrimFunc& f) {
  alloc_storage_scope_.clear();
  handle_data_type_.clear();
  CodeGenSourceBase::ClearFuncState();
}

void CodeGenRiscv::ReserveKeywordsAsUnique() {
...
void CodeGenRiscv::AddFunction(const PrimFunc& f) {
  ...
  ICHECK(global_symbol.defined())
      << "CodeGenRiscv: Expect PrimFunc to have the global_symbol attribute";
...
void CodeGenRiscv::PrintFuncPrefix() { stream << "void"; }

void CodeGenRiscv::PrintExtraAttrs(const PrimFunc& f) {}

void CodeGenRiscv::PrintFinalReturn() {}

std::string CodeGenRiscv::Finish() { return decl_stream.str() + stream.str(); }

void CodeGenRiscv::PrintExpr(const PrimExpr& n, std::ostream& os) {  // NOLINT(*)
  if (print_ssa_form_) {
...
void CodeGenRiscv::PrintSSAAssign(const std::string& target, const std::string& src, DataType t) {
  PrintType(t, stream);
  stream << ' ' << target << " = ";
...
std::string CodeGenRiscv::GetBufferRef(DataType t, const BufferNode* buffer, PrimExpr index) {
  ...
  if (t.bits() == 4 || (t.bits() == 1 && t.is_int())) {
    // This is a special case, because CodeGenCRiscvUDA::PrintType()
...
// Print a reference expression to a buffer.
std::string CodeGenRiscv::GetStructRef(DataType t, const PrimExpr& buffer, const PrimExpr& index,
                                       int kind) {
...
bool CodeGenRiscv::HandleTypeMatch(const VarNode* buf_var, DataType t) const {
  auto it = handle_data_type_.find(buf_var);
  if (it == handle_data_type_.end()) return false;
  return it->second == t;
}

void CodeGenRiscv::RegisterHandleType(const VarNode* buf_var, DataType t) {
  auto it = handle_data_type_.find(buf_var);
...
void CodeGenRiscv::PrintVecElemLoad(const std::string& vec, DataType t, int i,
                                    std::ostream& os) {  // NOLINT(*)
  os << vec << ".s" << std::hex << i << std::dec;
}

void CodeGenRiscv::PrintVecElemStore(const std::string& vec, DataType t, int i,
                                     const std::string& value) {
  this->PrintIndent();
  stream << vec << ".s" << std::hex << i << " = " << value << ";\n" << std::dec;
}

std::string CodeGenRiscv::GetVecLoad(DataType t, const BufferNode* buffer, PrimExpr base) {
  return GetBufferRef(t, buffer, base);
}

void CodeGenRiscv::PrintVecStore(const BufferNode* buffer, DataType t, PrimExpr base,
                                 const std::string& value) {
...
std::string CodeGenRiscv::CastFromTo(std::string value, DataType from, DataType target) {
...
void CodeGenRiscv::BindThreadIndex(const IterVar& iv) { LOG(FATAL) << "not implemented"; }

void CodeGenRiscv::PrintStorageSync(const CallNode* op) {  // NOLINT(*)
}

void CodeGenRiscv::PrintStorageScope(const std::string& scope, std::ostream& os) {  // NOLINT(*)
  ICHECK_EQ(scope, "global");
}

inline void PrintConst(const IntImmNode* op, std::ostream& os, CodeGenRiscv* p) {  // NOLINT(*)
...
inline void PrintConst(const FloatImmNode* op, std::ostream& os, CodeGenRiscv* p) {  // NOLINT(*)
...
void CodeGenRiscv::VisitExpr_(const IntImmNode* op, std::ostream& os) {  // NOLINT(*)
  PrintConst(op, os, this);
}

void CodeGenRiscv::VisitExpr_(const FloatImmNode* op, std::ostream& os) {  // NOLINT(*)
  PrintConst(op, os, this);
}
void CodeGenRiscv::VisitExpr_(const StringImmNode* op, std::ostream& os) {  // NOLINT(*)
  os << "\"" << op->value << "\"";
}
...
void CodeGenRiscv::VisitExpr_(const CastNode* op, std::ostream& os) {  // NOLINT(*)
  std::stringstream value;
  this->PrintExpr(op->value, value);
  os << CastFromTo(value.str(), op->value.dtype(), op->dtype);
}
void CodeGenRiscv::VisitExpr_(const VarNode* op, std::ostream& os) {  // NOLINT(*)
  os << GetVarID(op);
}
void CodeGenRiscv::VisitExpr_(const AddNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "+", os, this);
}
void CodeGenRiscv::VisitExpr_(const SubNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "-", os, this);
}
void CodeGenRiscv::VisitExpr_(const MulNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "*", os, this);
}
void CodeGenRiscv::VisitExpr_(const DivNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "/", os, this);
}
void CodeGenRiscv::VisitExpr_(const ModNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenRiscv::VisitExpr_(const MinNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "min", os, this);
}
void CodeGenRiscv::VisitExpr_(const MaxNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "max", os, this);
}
void CodeGenRiscv::VisitExpr_(const EQNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "==", os, this);
}
void CodeGenRiscv::VisitExpr_(const NENode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "!=", os, this);
}
void CodeGenRiscv::VisitExpr_(const LTNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "<", os, this);
}
void CodeGenRiscv::VisitExpr_(const LENode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "<=", os, this);
}
void CodeGenRiscv::VisitExpr_(const GTNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, ">", os, this);
}
void CodeGenRiscv::VisitExpr_(const GENode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, ">=", os, this);
}
void CodeGenRiscv::VisitExpr_(const AndNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "&&", os, this);
}
void CodeGenRiscv::VisitExpr_(const OrNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "||", os, this);
}
void CodeGenRiscv::VisitExpr_(const NotNode* op, std::ostream& os) {  // NOLINT(*)
  os << '!';
  PrintExpr(op->a, os);
}

void CodeGenRiscv::PrintCallExtern(Type ret_type, String global_symbol, const Array<PrimExpr>& args,
  ...
  os << global_symbol << "(";
  for (size_t i = static_cast<size_t>(skip_first_arg); i < arg_size; ++i) {
...
void CodeGenRiscv::VisitExpr_(const CallNode* op, std::ostream& os) {  // NOLINT(*)
  if (auto* ptr_op = op->op.as<OpNode>()) {
    ...
    if (op->op.same_as(builtin::tvm_check_return())) {
      ...
      ICHECK_EQ(load->indices.size(), 1) << "CodeGenRiscv only supports flat memory allocations.";
...
void CodeGenRiscv::PrintVecBinaryOp(const std::string& op, DataType t, PrimExpr lhs, PrimExpr rhs,
                                    std::ostream& os) {  // NOLINT(*)
  if (isalpha(op[0])) {
    os << op << "(";
...
void CodeGenRiscv::VisitStmt_(const AllocateConstNode* op) {
...
void CodeGenRiscv::VisitExpr_(const LoadNode* op, std::ostream& os) {  // NOLINT(*)
  LOG(FATAL) << "Unexpected deprecated LoadNode.  Use BufferLoadNode instead.";
}

void CodeGenRiscv::VisitExpr_(const BufferLoadNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenRiscv::VisitStmt_(const StoreNode* op) {
  LOG(FATAL) << "Unexpected deprecated StoreNode.  Use BufferStoreNode instead.";
}

void CodeGenRiscv::VisitStmt_(const BufferStoreNode* op) {
...
void CodeGenRiscv::VisitExpr_(const LetNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenRiscv::VisitExpr_(const RampNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenRiscv::VisitExpr_(const ShuffleNode* op, std::ostream& os) {
  LOG(FATAL) << "Shuffle: not supported ";
}

void CodeGenRiscv::VisitExpr_(const BroadcastNode* op, std::ostream& os) {  // NOLINT(*)
  LOG(FATAL) << "Broadcast: not supported ";
}

void CodeGenRiscv::VisitExpr_(const SelectNode* op, std::ostream& os) {  // NOLINT(*)
  os << "(";
...
void CodeGenRiscv::VisitStmt_(const LetStmtNode* op) {
  std::string value = PrintExpr(op->value);
  if (print_ssa_form_) {
...
void CodeGenRiscv::VisitStmt_(const AllocateNode* op) {
  ICHECK(!is_zero(op->condition));
  std::string vid = AllocVarID(op->buffer_var.get());
  auto scope = GetPtrStorageScope(op->buffer_var);
  if (vid.substr(vid.size() - 6) != "global") {
    this->PrintIndent();
...
void CodeGenRiscv::VisitStmt_(const AttrStmtNode* op) {
...
void CodeGenRiscv::VisitStmt_(const AssertStmtNode* op) {
...
void CodeGenRiscv::VisitStmt_(const ForNode* op) {
...
void CodeGenRiscv::VisitStmt_(const WhileNode* op) {
  PrintIndent();
...
void CodeGenRiscv::VisitStmt_(const IfThenElseNode* op) {
  std::string cond = PrintExpr(op->condition);
  PrintIndent();
...
void CodeGenRiscv::VisitStmt_(const SeqStmtNode* op) {
  for (Stmt stmt : op->seq) {
    PrintStmt(stmt);
  }
}

void CodeGenRiscv::VisitStmt_(const EvaluateNode* op) {
...
void CodeGenRiscv::PrintVecElemLoadExpr(DataType t, int i, const std::string& value,
                                        std::ostream& os) {
  ICHECK_GT(t.lanes(), 1);
...
void CodeGenRiscv::PrintRestrict(const Var& v, std::ostream& os) {
