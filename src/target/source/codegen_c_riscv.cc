...
CodeGenCRiscv::CodeGenCRiscv() { module_name_ = GetUniqueName("__tvm_module_ctx"); }

void CodeGenCRiscv::Init(bool output_ssa, bool emit_asserts, std::string target_str,
  ...
  CodeGenRiscv::Init(output_ssa);
}

void CodeGenCRiscv::InitGlobalContext() {
  decl_stream << "void* " << tvm::runtime::symbol::tvm_module_ctx << " = NULL;\n";
}

void CodeGenCRiscv::DefineModuleName() { decl_stream << "void* " << module_name_ << " = NULL;\n"; }

void CodeGenCRiscv::AddFunction(const PrimFunc& f) {
  auto global_symbol = f->GetAttr<String>(tvm::attr::kGlobalSymbol);
  ICHECK(global_symbol.defined())
      << "CodeGenCRiscv: Expect PrimFunc to have the global_symbol attribute";
  function_names_.push_back(global_symbol.value());

  CodeGenRiscv::AddFunction(f);
  if (f->HasNonzeroAttr(tir::attr::kIsEntryFunc)) {
    function_names_.push_back(runtime::symbol::tvm_module_main);
    stream << "// CodeGenRiscv: NOTE: Auto-generated entry function\n";
    PrintFuncPrefix();
...
void CodeGenCRiscv::PrintFuncPrefix() {  // NOLINT(*)
...
void CodeGenCRiscv::PrintFinalReturn() {  // NOLINT(*)
  this->PrintIndent();
  stream << "return 0;\n";
}

void CodeGenCRiscv::PrintType(DataType t, std::ostream& os) {  // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
...
void CodeGenCRiscv::VisitExpr_(const BroadcastNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenCRiscv::PrintGetFuncFromBackend(const std::string& func_name,
...
void CodeGenCRiscv::PrintFuncCall(const std::string& packed_func_name, int num_args) {
  this->PrintIndent();
...
void CodeGenCRiscv::PrintFuncCallC(const std::string& packed_func_name, int num_args,
...
std::string CodeGenCRiscv::GetPackedName(const CallNode* op) {
...
CodeGenCRiscv::FunctionInfo CodeGenCRiscv::GetFunctionInfo(const CallNode* op,
...
void CodeGenCRiscv::VisitExpr_(const CallNode* op, std::ostream& os) {  // NOLINT(*)
  if (op->op.same_as(builtin::tvm_stack_alloca())) {
    ...
    CodeGenRiscv::VisitExpr_(op, os);
  }
}

void CodeGenCRiscv::VisitStmt_(const AssertStmtNode* op) {  // NOLINT(*)
  if (emit_asserts_) {
...
void CodeGenCRiscv::VisitExpr_(const MinNode* op, std::ostream& os) {  // NOLINT(*)
  PrintTernaryCondExpr(op, "<", os);
}

void CodeGenCRiscv::VisitExpr_(const MaxNode* op, std::ostream& os) {  // NOLINT(*)
  PrintTernaryCondExpr(op, ">", os);
}

template <typename T>
inline void CodeGenCRiscv::PrintTernaryCondExpr(const T* op, const char* compare,
...
runtime::Module BuildCRiscv(IRModule mod, Target target) {
  ...
  for (auto& kv : funcs) {
    ICHECK(kv.second->IsInstance<PrimFuncNode>()) << "CodeGenCRiscv: Can only take PrimFunc";
...
TVM_REGISTER_GLOBAL("target.build.riscv").set_body_typed(BuildCRiscv);
