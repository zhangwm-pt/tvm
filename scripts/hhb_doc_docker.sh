...
docker run --rm -v $script_dir/../../tvm:/mnt hhb4tools/hhb_build:0.5 sh -c "cd mnt/ && rm build -rf  &&  mkdir build  &&  cd build  &&  cp ../cmake/config.cmake . && echo 'set(USE_MICRO ON)' >> config.cmake &&  cmake ..  &&  make -j32"
